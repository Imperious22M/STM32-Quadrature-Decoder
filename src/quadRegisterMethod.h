...
void setupQuadTimers(){
    ...
    //GPIOA->AFR[0]  |= GPIO_AFRL_AFRL0_2 ;                                          //  AF01 for PA0 & PA1              /*!< GPIO alternate function registers,     Address offset: 0x20-0x24 */
    //GPIOA->AFR[1]  |= 0x00000000 ;                                          //                                  /*!< GPIO alternate function registers,     Address offset: 0x20-0x24 */
    GPIOA->MODER   |= GPIO_MODER_MODER0_1 | GPIO_MODER_MODER1_1 ;           //PB4 & PB5 as Alternate Function   /*!< GPIO port mode register,               Address offset: 0x00      */
    GPIOA->OTYPER  |= GPIO_OTYPER_OT_0 | GPIO_OTYPER_OT_1 ;                 //PB4 & PB5 as Inputs               /*!< GPIO port output type register,        Address offset: 0x04      */
    GPIOA->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR0 | GPIO_OSPEEDER_OSPEEDR1;      //0x11000000;//                        /*!< GPIO port output speed register,       Address offset: 0x08      */
    GPIOA->PUPDR   |= GPIO_PUPDR_PUPDR0_1 | GPIO_PUPDR_PUPDR1_1 ;           // Pull Down                        /*!< GPIO port pull-up/pull-down register,  Address offset: 0x0C      */
    GPIOA->AFR[0]  |= GPIO_AFRL_AFRL0_1 | GPIO_AFRL_AFRL1_1;//0x11000011 ;                                          //  AF01 for PB4 & PB5              /*!< GPIO alternate function registers,     Address offset: 0x20-0x24 */
    GPIOA->AFR[1]  |= 0x00000000 ;                                          //                                  /*!< GPIO alternate function registers,     Address offset: 0x20-0x24 */

    // configure TIM5 as Encoder input
    RCC->APB1ENR |= RCC_APB1ENR_TIM5EN;  // Enable clock for TIM5
 
    TIM5->CR1   = TIM_CR1_CEN;     // CEN(Counter ENable)='1'     < TIM control register 1
    TIM5->SMCR  = 0x0003;   // SMS='011' (Encoder mode 3)  < TIM slave mode control register
    TIM5->CCMR1 = 0x0101;     // CC1S='01' CC2S='01'         < TIM capture/compare mode register 1
    ...
    TIM5->ARR   = 0xffff; // reload at 0xfffffff         < TIM auto-reload register
    //Tim5->BDTR
    TIM5->CNT = 0x8000;  //reset the counter before we use it 
    ...
    TIM3->ARR   = 0xffff; // reload at 0xfffffff         < TIM auto-reload register
    //Tim3->BDTR
...
uint32_t readQuadA(){
    uint32_t ret = (uint16_t)TIM5->CNT; // Treat the encoder as a 16-bit timer (even if TIM2 is a 32 bit timer)
    TIM5->CNT = 0x8000;
    return ret;
}
uint32_t readQuadB(){
...
bool readDirQuadA(){
    return TIM5->CR1 & TIM_CR1_DIR; // DOUBLE CHECK!!
}
bool readDirQuadB(){
